...
const service = axios.create({
  ...
  //baseURL: '/myselfApi',
  //baseURL:`${window.location.protocol}//${window.location.hostname}:80/api/`,
...
service.interceptors.request.use(
  config => {
    if (!config.headers["noLang"]) {
      config.url += '/' + Cookies.get('language')
    }
    if (getToken()) {
...
service.interceptors.response.use(
  ...
  error => {
    ...
    if(error.response&&error.response.status=='401'){
      ...
      if(!Cookies.get('tokeninvalid')){
        Message.warning('The token is invalid')
        Cookies.set('tokeninvalid',1)
      }
      location.reload()
    }
