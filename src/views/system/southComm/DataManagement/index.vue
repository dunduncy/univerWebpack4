    ...
    <div class="box_container pd">
        <div class="tablefilterContainer">
            <div class="common_con_div">
                <span class="conditionName">所属南向总线：</span>
        ...
        <div class="content">
            <div class="table_container">
                <el-table :data="list" ref="multipleTable" stripe style="width: 100%;" height="100%" row-key="id" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="位偏移量" align="center" prop="bitOffset" />
                    <el-table-column label="位个数" align="center" prop="bitCount" />
                    <el-table-column label="寄存器地址" align="center" prop="registerAddress" />
                    <el-table-column label="寄存器数量" align="center" prop="registerCount" v-if="protocolType !== 4" />
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false"  width="40%">
            <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="auto" class="selfDefineCenterForm">
                ...
                <el-form-item label="字节序" prop="endian">
                ...
                <el-form-item label="位偏移量" prop="bitOffset">
                    <el-input v-model.number="temp.bitOffset" :disabled="isDisabled" />
                </el-form-item>
                <el-form-item label="位个数" prop="bitCount">
                    <el-input v-model.number="temp.bitCount" :disabled="isDisabled" />
                </el-form-item>
                <el-form-item label="寄存器地址" prop="registerAddress">
                    <el-input v-model.number="temp.registerAddress" :disabled="isDisabled" />
                </el-form-item>
                <el-form-item label="寄存器数量" prop="registerCount" v-if="protocolType !== 4">
                    <el-input v-model.number="temp.registerCount" :disabled="isDisabled" />
                </el-form-item>
...
import {
    ...
    add104SouthData, update104SouthData, delete104SouthData, list104SouthData, dataCopy104,
    addModbusSouthData, updateModbusSouthData, deleteModbusSouthData, listModbusSouthData, dataCopyModbus
} from '@/api/system/southData.js'
export default {
    data() {
        ...
        return {
            ...
            rules: {
                ...
                bitCount: [
                    { required: true, message: '位个数不能为空', trigger: 'blur' },
                    { validator: validateNumber, trigger: 'blur' }
                ],
                dataName: [
    ...
    methods: {
        ...
        getList() {
            ...
            let fun = this.protocolType == 4 ? list104SouthData : listModbusSouthData
            fun(data).then(res => {
        ...
        handleDelete(row, index) {
            this.$confirm(this.$t('commonNotify.DeleteInfoMessage'), this.$t('commonNotify.Prompt'), {
                ...
                .then(async () => {
                    try {
                        let fun = this.protocolType==4?delete104SouthData:deleteModbusSouthData
                        const res = await fun({ southDataId: row.id })
                        if (res.code == 1) {
                            this.$notify({
                ...
                if (valid) {
                    ...
                    if (this.dialogStatus == 'create') {
                        if (this.protocolType == 4) {
                            ...
                            fun = addModbusSouthData
                        }
                    } else {
                        if (this.protocolType == 4) {
                            fun = update104SouthData
                        } else {
                            fun = updateModbusSouthData
                        }
                    }
