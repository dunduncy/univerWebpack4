  ...
  <div class="content_box">
      ...
      <div class="box_outter">
        <div class="table_title">
          <div style="float: right;margin-bottom: 10px;">
            <span style="font-size: 14px;">{{ $t('system$systemView$param$index_vue.timeprecision') }}：</span>
          <el-select size="small" v-model="timeSpan" @change="changeTimePrecision">
              <el-option  v-for="item in options" :key="item.value" :label="currentLang == 'en'?item.labelEn:item.labelCh" :value="item.value"></el-option>
          </el-select>
          </div>
        ...
        <div class="table_Container">
          <el-table :data="tableData" stripe style="width: 100%;" height="100%" row-key="id">
            <el-table-column prop="paramNameEn" :label="$t('system$systemView$rtData$Battery$index_vue.parametername')"
              align="center" v-if="currentLang == 'en'">
            </el-table-column>
            <el-table-column prop="paramNameCh" :label="$t('system$systemView$rtData$Battery$index_vue.parametername')"
              align="center" v-else>
            </el-table-column>
            <el-table-column prop="value" :label="$t('system$systemView$rtData$Battery$index_vue.parametervalue')"
              align="center" :formatter="formatValue">
            </el-table-column>
            <el-table-column prop="operate" :label="$t('system$systemView$rtData$Battery$index_vue.operate')"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false"
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="auto"
        ...
        <el-form-item :label="$t('system$systemView$rtData$Battery$index_vue.parametervalue')" prop="value" :rules="[
        ...
        <el-form-item :label="$t('system$systemView$rtData$Battery$index_vue.parametervalue')" prop="value"
          v-else-if="currentRow.dataType == 'string'">
          <el-input v-model="temp.value" type="string" />
        </el-form-item>
        <el-form-item :label="$t('system$systemView$rtData$Battery$index_vue.parametervalue')" prop="value"
          v-if="currentRow.dataType == 'bool'">
          <el-switch v-model="temp.value" active-text="true" inactive-text="false"></el-switch>
        </el-form-item>
        <el-form-item :label="$t('system$systemView$rtData$Battery$index_vue.parametervalue')" prop="value"
          v-if="currentRow.dataType == 'boolEnum' || currentRow.dataType == 'numberEnum'">
          <el-select v-model="temp.value">
            <el-option v-for="item in currentRow.enumRule" :key="item.value"
              :label="currentLang == 'en' ? item.labelEn : item.labelCh" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
import { getPlanCurveTimeSpan, getPlanCurve, setPlanCurve } from '@/api/system/param'
  ...
  methods: {
    ...
    formatValue(row, column, cellValue, index) {
      if (row.dataType == 'number') {
        return this.handleValue(cellValue, row.precesion || 2)
    ...
    async getPlanCurveTimeSpan(){
      const res = await getPlanCurveTimeSpan()
      if(res.code==1 && res.data&&res.data.length>0){
        this.timeSpan = res.data[0].value
        this.options = res.data
        this.getList()
      }
    },
    async getList() {
      const res = await getPlanCurve({timeSpan:this.timeSpan})
