  ...
  <div class="content_box">
    <div class="box_left">
      <div class="box_outter">
        <div class="table_title">{{ $t('system$systemView$rtData$Battery$index_vue.batteryoverview') }}</div>
    ...
    <div class="box_right">
      <div class="box_outter">
        <div class="table_title">{{ $t('system$systemView$rtData$Battery$index_vue.batteryalarm') }}</div>
        <div class="table_Container">
          <el-table :data="tableData2" stripe style="width: 100%;" height="100%" row-key="id">
            <el-table-column prop="time" :label="$t('system$systemView$rtData$Battery$index_vue.alarmtriggertime')" width="180" align="center" :formatter="formatTime">
            </el-table-column>
            <el-table-column prop="englishName" :label="$t('system$systemView$rtData$Battery$index_vue.alarmname')" v-if="currentLang=='en'">
            </el-table-column>
            <el-table-column prop="chinese" :label="$t('system$systemView$rtData$Battery$index_vue.alarmname')" v-else>
            </el-table-column>
            <el-table-column prop="value" :label="$t('system$systemView$rtData$Battery$index_vue.alarmvalue')">
            </el-table-column>
...
<script>
import { listBatteryOverviewData, listBatteryYXData } from '@/api/system/rtData'
...
export default {
  ...
  methods: {
    ...
    async getList1(isloading=false) {
      ...
      const { data:data1 } = await listBatteryOverviewData(data,isloading)
    ...
    async getList2(isloading=false) {
      ...
      const { data:data1 } = await listBatteryYXData(data,isloading)
      const { records, total } = data1
